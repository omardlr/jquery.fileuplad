$.fn.fileupload = function (url, opt) {
    ...
    */     
    var $this = this; //this...
    var $fobject = this.find("input[type='file']"); //input type file that contains the files...
    var max = this.find("input[type='file']")[0].files.length; //total files
    var min = 0; //for configurations...
    var now = 0; //current [index] file
    var step = 0; //how much we go to advance by each file that be processed....
    var files = this.find("input[type='file']")[0].files; //files...
    var current = 0; //current index of the file to be processed
    var url = url || ""; //where we goin to send the file? ahhhhh url......
    var imgToUpload = {}; //collection of images to be processed
    var Api = {}; //for now it is just for delete a image from de collection.....
